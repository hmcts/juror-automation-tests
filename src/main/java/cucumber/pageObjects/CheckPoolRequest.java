...
//@FindBy(xpath = "//div[@class='govuk-summary-list__row']/dt[contains(text(),'Pool number')]/following-sibling::dd[1]")
    ...
    public String getPoolNumber() {
        ...
        while (retryCount < maxRetries) {
            try {
                String value = poolNumber.getText();
                log.info("Pool number retrieved: " + value);
                return value;
            } catch (StaleElementReferenceException e) {
                lastException = e;
                retryCount++;
                log.warn("StaleElementReferenceException when getting pool number - attempt " + retryCount);
        ...
        log.error("Unable to get pool number due to stale element: " +
                (lastException != null ? lastException.getMessage() : "unknown"));
        throw new RuntimeException("Unable to retrieve pool number after retries.");
    }
    ...
    public void submitRequest() {
        ...
        while (retryCount < maxRetries) {
            try {
                ...
                lastException = e;
                retryCount++;
                log.warn("StaleElementReferenceException when submitting request - attempt " + retryCount);
        ...
        log.error("Unable to submit request due to stale element: " +
                (lastException != null ? lastException.getMessage() : "unknown"));
        throw new RuntimeException("Failed to submit request after retries.");
    }
}
