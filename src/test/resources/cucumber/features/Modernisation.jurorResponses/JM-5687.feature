Feature: JM-5687 - Resend Confirmation letter
  ...
  Scenario Outline:As a bureau officer test a excused juror can resend a confirmation letter by searching via Pool number - unhappy path
  ...
  #search via pool  number and resend letter for first excused juror
    ...
    And I set "Enter pool number" to "<pool_number>"
    And I press the "Search" button
    Then I see "<juror_number>" in the same row as "Responded"
    #Script will fail due to JM-6227 - and possibly another
    And I see "<juror_number_2>" in the same row as "Excused"

    Examples:
      | juror_number  |juror_number_2  |pool_number   | user          |
      |  041588981    | 041588982      |415981288     | MODTESTBUREAU |
