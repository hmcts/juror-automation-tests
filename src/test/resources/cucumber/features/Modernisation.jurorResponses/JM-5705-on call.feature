Feature: As a jury officer I need to send a on call message
  ...
  Scenario Outline: Send on call message for Juror using Text including error checks
    ...
    And I click on the "On call" link
    And I see "Message details" on the page
    And I see the draft message template
    And I see "You are not required to attend for Jury Service on <attend_date>. We will be in touch by text/email with regards to your next attendance. If you have any questions, please contact the jury office on <court_phone>." on the page
