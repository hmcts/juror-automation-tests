Feature: As a jury officer I need to send the selected for a panel message
  ...
  Scenario Outline: Send selected for a panel message for Juror using Text including error checks
    ...
    And I click on the "Selected for panel" link
  ...
  Scenario Outline: Send selected for a panel message for Juror
    ...
    And I see "Message details" on the page
    And I see the draft message template
    And I see "You have been selected to form a panel from which a jury will be chosen. Please attend <court_name> Court on <attend_date> at <attend_time>. If you have any questions, please contact the jury office on <court_phone>." on the page
    And I set the "Attendance date for message" date to a Monday "5" weeks in the future
    And I set "Hour" to "09"
    And I set "Minute" to "00"
#    And I set the radio button to "am"
