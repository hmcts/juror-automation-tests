...
public class StepDef_jurorpoolNewSchema {
    ...
    public StepDef_jurorpoolNewSchema(CoronersPool coroners_pool_page, SharedDriver webDriver, DatabaseTesterNewSchemaDesign dbtnsd) {
...
//    }
//
//    @When("^I insert a valid pool number$")
//    public void inputValidPoolNumber() {
//        String shortPoolNo = POOL_REQUESTS_PAGE.getExistingPoolNumber().substring(0, 7);
//        String newPoolNo = shortPoolNo + "99";
//        poolNumber.set(newPoolNo);
//        POOL_REQUESTS_PAGE.inputValidPoolNumber(newPoolNo);
//    }

    @When("^I insert a valid pool number for court \"([^\"]*)\"$")
    public void inputValidPoolNumberNSD(String locCode) throws SQLException {
        ...
        String newPoolNo = locCode + year + month;
        String incrementedPoolNo = String.valueOf(DBTNSD.getNextPoolNoNSD(newPoolNo));
        poolNumber.set(incrementedPoolNo);
        POOL_REQUESTS_PAGE.inputValidPoolNumber(incrementedPoolNo);
    }
//
...
//    }
//
    @When("^I create a bank holiday \"([^\"]*)\" weeks in the future for court/bureau \"([^\"]*)\" new schema$")
    public void insertHolidayInTheFutureNSD(Integer noOfWeeks, String holidayOwner) throws SQLException {
    ...
    public void insertHolidayMondayInTheFutureNSD(Integer noOfWeeks, String holidayOwner) throws SQLException {
        DBTNSD.insertHolidayMondayInTheFutureNSD(noOfWeeks, holidayOwner);
    }

    @When("^I delete bank holiday new schema")
    public void iDeleteBankHolidayNSD() throws SQLException {
        DBTNSD.deleteHolidayNSD();
    }

    @When("^I have deleted all holidays new schema")
    public void deleteAllHolidaysNSD() throws SQLException {
        DBTNSD.deleteAllHolidaysNSD();
    }
//
...
//    }
//
    @Given("^a juror record of \"([^\"]*)\" has been entered which is valid for paper summons reply new schema$")
    public void createPaperSummonsReplyJuror(String newJurorRecordNumber) throws SQLException {
        DBTNSD.insertNewPaperSummonsReplyNSD(newJurorRecordNumber, "415230501");
        summonsReplyNumber = newJurorRecordNumber;
    }

    @Given("^I clear down the data for the paper juror record \"([^\"]*)\" new schema$")
    public void deletePaperSummonsJurorNSD(String createdJurorRecordNumber) throws SQLException {
        DBTNSD.deleteNewPaperSummonsReplyNSD(createdJurorRecordNumber);
    }


    @Given("^I clear down the data for the pool new schema$")
    public void deleteTestPoolAndJurorsNSD() throws SQLException {
        DBTNSD.deleteTestPoolAndJurorsNSD(poolNumber.get());
    }
//
...
//
    ...
    public void jurorRecordDisplayedWithFollowingFields(DataTable table) {
        ...
        assertEquals(DBTNSD.newPoolDate.get() , JUROR_RECORD_SEARCH.getCourtStartDate());
...
//        SUMMONS_REPLY.clickSignedYes();
//        SUMMONS_REPLY.clickDoneButton();
//        SUMMONS_REPLY.clickProcessNowButton();
//    }
//
...
//    }
    ...
    public void checkNewJurorSummonsLetterNSD(String noJurors) throws SQLException {
        ...
        for (int index = 1; index <= jurorRows; index++) {
            ...
            assertEquals("Juror summons letter sent to " + jurorNo, DBTNSD.checkJurorSummonsLetterNSD(jurorNo));
        }
    }
//
